...
*)
let help (l:('a list, 'e) result)(x:('a, 'e) result) : ('a list, 'e) result=
match l with
| Ok(o) -> (
match x with
| Ok(v) -> ok(v::o)
...
let all_ok (ls : ('a, 'e) result list) : ('a list, 'e) result =
  match fold_left help (ok([])) ls with
  | Ok(o) -> ok(reverse(o))
  | Err(oo) -> Err(oo)
...
let example_stream =    LPAREN ^ LPAREN^ SYMBOL("+") ^ NUMBER(3) ^ NUMBER(2) ^ RPAREN ^ RPAREN^ eof ()
let example_stream =    LPAREN ^ LPAREN^ SYMBOL("+") ^ NUMBER(3) ^ NUMBER(2) ^ RPAREN ^ RPAREN^ eof ()
...
let rec parse_sexpr (st : token stream): (sexpr * token stream, parse_err) result =
  ...
  let ta=tail(st) in
  match hd with
  |EOF -> ok(SList(snil),ta)
  |SYMBOL(s) -> ok(ssymbol s,ta)
  |NUMBER(n) -> ok(snumber n,ta)
  |LPAREN -> (match parse_sexpr_list ta with
  | Ok(sl,re) -> ok(SList(sl),re)
  | Err(b) -> err(b))
...
|SYMBOL(s) -> (match parse_sexpr_list ta with
| Ok(sl,re) -> ok(ssymbol s::sl,re)
| Err(b) -> err(b))
|NUMBER(n) -> (match parse_sexpr_list ta with
| Ok(sl,re) -> ok(snumber n::sl,re)
| Err(b) -> err(b))
|LPAREN -> (match parse_sexpr_list ta with
| Ok(sl,re) ->  (match parse_sexpr_list re with
| Ok(sl2,re2) -> ok(SList(sl)::sl2,re2)
| Err(b) -> err(b))
| Err(b) -> err(b))
|RPAREN -> ok([],ta)
