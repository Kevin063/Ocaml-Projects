...
let rec check_prime (n : int)(m : int) : bool=
   match m with
    |1 -> true
    |m -> (n mod m !=0)&&check_prime(n)(m-1)
let is_prime (n : int) : bool = check_prime(n)(n-1)
(* Problem 4.
   Task:
...
let rec all_primes (n : int) : string = 
   if n==2 then "2"
   else if is_prime(n) then all_primes(n-1)^" "^string_of_int(n)
   else all_primes(n-1);;
...
let rec bin_of_int (n : int) : string = 
   match n with
    ...
    |2 -> "10"
    |1 -> "1"
    |n -> bin_of_int(n/2)^string_of_int(n mod 2);;
